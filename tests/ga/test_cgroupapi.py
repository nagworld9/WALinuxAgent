...
from azurelinuxagent.ga.cgroupapi import SystemdCgroupApiv1, SystemdCgroupApiv2, CGroupUtil, create_cgroup_api, \
...
class SystemdCgroupsApiTestCase(AgentTestCase):
    def test_create_cgroup_api_raises_exception_when_systemd_mountpoint_does_not_exist(self):
    ...
    def test_create_cgroup_api_is_v2_when_v2_in_use(self):
        with mock_cgroup_v2_environment(self.tmp_dir):
            self.assertIsInstance(create_cgroup_api(), SystemdCgroupApiv2)

    def test_create_cgroup_api_raises_exception_when_hybrid_in_use_and_controllers_available_in_unified_hierarchy(self):
    ...
    def test_create_cgroup_api_raises_exception_when_v1_in_use_and_controllers_have_non_sytemd_mountpoints(self):
        with mock_cgroup_v1_environment(self.tmp_dir):
            # Mock /sys/fs/cgroup/unified/cgroup.controllers file to have available controllers
    ...
    def test_create_cgroup_api_is_v1_when_v1_in_use(self):
        with mock_cgroup_v1_environment(self.tmp_dir):
            self.assertIsInstance(create_cgroup_api(), SystemdCgroupApiv1)

    def test_create_cgroup_api_is_v1_when_hybrid_in_use(self):
        with mock_cgroup_hybrid_environment(self.tmp_dir):
            # Mock os.path.exists to return True for the os.path.exists('/sys/fs/cgroup/cgroup.controllers') check
            with patch("os.path.exists", return_value=True):
                self.assertIsInstance(create_cgroup_api(), SystemdCgroupApiv1)

    def test_create_cgroup_api_raises_exception_when_cgroup_mode_cannot_be_determined(self):
...
class SystemdCgroupsApiv1TestCase(AgentTestCase):
    ...
    def test_get_relative_paths_for_process_should_return_the_cgroup_v1_relative_paths(self):
        with mock_cgroup_v1_environment(self.tmp_dir):
            relative_paths = create_cgroup_api()._get_process_relative_controller_paths('self')
    ...
    def test_get_unit_cgroup_should_return_correct_paths_for_cgroup_v1(self):
        with mock_cgroup_v1_environment(self.tmp_dir):
            cgroup = create_cgroup_api().get_unit_cgroup(unit_name="extension.service", cgroup_name="extension")
    ...
    def test_get_cgroup_from_relative_path_should_return_the_correct_paths_for_cgroup_v1(self):
        with mock_cgroup_v1_environment(self.tmp_dir):
            cgroup = create_cgroup_api().get_cgroup_from_relative_path(relative_path="some/relative/path", cgroup_name="test_cgroup")
            self.assertIsInstance(cgroup, CgroupV1)
        ...
        with mock_cgroup_v1_environment(self.tmp_dir):
            ...
            with tempfile.TemporaryFile(dir=self.tmp_dir, mode="w+b") as output_file:
                with patch("subprocess.Popen",
                    ...
                    command_output = create_cgroup_api().start_extension_command(
    ...
    def test_start_extension_cgroups_v1_command_should_execute_the_command_in_a_cgroup(self, _):
        with mock_cgroup_v1_environment(self.tmp_dir):
            create_cgroup_api().start_extension_command(
                extension_name="Microsoft.Compute.TestExtension-1.2.3",
