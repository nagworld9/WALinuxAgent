...
class TestEvent(HttpRequestPredicates, AgentTestCase):
    ...
    @staticmethod
    def _collect_events(return_event_record=False):
        def append_event(e):
            # Event is member of TelemetryEventInfo
            for p in e.event.parameters:
    ...
    def test_add_event_should_use_the_container_id_from_the_most_recent_goal_state(self):
        def create_event_and_assert_new_container_id(count, containerId):  # pylint: disable=inconsistent-return-statements
            event.add_event(name='Event', op=TestEvent._Operation)
            event_list = self._collect_events()
            self.assertEqual(len(event_list), count, "Could not find the event created by add_event")

            for e in event_list:
                for p in e.parameters:
                    if p.name == CommonTelemetryEventSchema.ContainerId:
                        if p.value == containerId:
                            return
        ...
        with mock_wire_protocol(wire_protocol_data.DATA_FILE) as protocol:
            ...
            create_event_and_assert_new_container_id(count, "c6d5526c-5ac2-4200-b6e2-56f2b70c5ab2")

            protocol.mock_wire_data.set_container_id('AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE')
            protocol.client.update_goal_state()
            count += 1
            create_event_and_assert_new_container_id(count, "AAAAAAAA-BBBB-CCCC-DDDD-EEEEEEEEEEEE")

            protocol.mock_wire_data.set_container_id('11111111-2222-3333-4444-555555555555')
            protocol.client.update_goal_state()
            count += 1
            create_event_and_assert_new_container_id(count, "11111111-2222-3333-4444-555555555555")
    ...
    def test_collect_events_should_not_delete_event_files(self):
        ...
        # protocal initialization creating additional events along with test events, so event may greater than 3
        self.assertGreaterEqual(len(event_files), 3, "The event files were deleted")
    ...
    def test_collect_events_should_ignore_invalid_event_files(self):
        ...
        with patch("azurelinuxagent.common.event.add_event") as mock_add_event:
            ...
            with patch("azurelinuxagent.ga.collect_telemetry_events.NUM_OF_EVENT_FILE_RETRIES", 1):
                event_list = self._collect_events()
                event_files = self._collect_test_event_files("custom_script")

                # The invalid event files should be removed
                self.assertEqual(2, len(event_files), "The invalid event files were not removed")
                self.assertEqual(
                    len(event_list), 2)
                self.assertTrue(
    ...
    def test_report_event_should_encode_call_stack_correctly(self):
        ...
        with mock_wire_protocol(wire_protocol_data.DATA_FILE, http_post_handler=http_post_handler) as protocol:
            ...
            event_list = self._collect_events(return_event_record=True)
    ...
    def test_report_event_should_delete_event_files_after_sending(self):
        def http_post_handler(url, body, **__):
            if self.is_telemetry_request(url):
                http_post_handler.request_body = body
                return MockHttpResponse(status=200)
        ...
        with mock_wire_protocol(wire_protocol_data.DATA_FILE, http_post_handler=http_post_handler) as protocol:
            add_event('Report Event', message='Test Message', op=TestEvent._Operation)
            add_event('Report Event', message='Test Message2', op=TestEvent._Operation)
            event_list = self._collect_events(return_event_record=True)
            event_files = self._collect_event_files()
            self.assertEqual(2, len(event_list), "Did not collect all the events that were created")
            self.assertEqual(2, len(event_files), "Did not find all the event files that were created")

            self._report_events(protocol, event_list)
            event_files = self._collect_event_files()
            self.assertEqual(len(event_files), 0, "The event files were not deleted")

    def test_report_event_should_delete_not_event_files_if_fail_to_send(self):
        def http_post_handler(url, body, **__):
            if self.is_telemetry_request(url):
                http_post_handler.request_body = body
                return MockHttpResponse(status=500)
        ...
        with mock_wire_protocol(wire_protocol_data.DATA_FILE, http_post_handler=http_post_handler) as protocol:
            ...
            self._report_events(protocol, event_list)
            event_files = self._collect_event_files()
            self.assertEqual(2, len(event_files), "The event files were deleted")
